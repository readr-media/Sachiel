...
export default function NotificationDropdown({
      ...
      return CommonContainer(
        ...
        renderNotifierText(
          ...
          <span className="text-primary-600">
            {t(
              'Components.NotificationDropdown.start-following-you',
              '開始追蹤你'
            )}
          </span>,
          <span className="text-primary-600">
            {t(
              'Components.NotificationDropdown.all-start-following-you',
              '都開始追蹤你'
            )}
          </span>
        ),
        ...
        if ('publisher' in parsedContent) {
          return CommonContainer(
            ...
            <p className="body-3 text-primary-600">
              {t(
                'Components.NotificationDropdown.sponsor-success',
                '你已成功贊助「{{title}}」{{fee}} 讀選點數',
                {
                  title: parsedContent.publisher.title,
        ...
        if ('title' in parsedContent) {
          return CommonContainer(
            ...
            <p className="body-3 text-primary-700">
              ...
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.created-new-collection',
                  '建立了新的集錦「{{title}}」',
      ...
      case 'approach_expiration:transaction':
        if ('expireDate' in parsedContent) {
          return CommonContainer(
            ...
            <p className="body-3 text-primary-600">
              {t(
                'Components.NotificationDropdown.reading-period-expiring',
                '這篇文章的閱讀期限只剩{{days}}天哦：「{{title}}」',
                {
                  days: displayExpireTimeFromNow(parsedContent.expireDate),
                  title: parsedContent.unlockStory.title,
                }
              )}
            </p>,
            time,
      ...
      case 'notify_transaction:transaction':
        if (
          ...
          return CommonContainer(
            ...
            <p className="body-3 text-primary-600">
              {t(
                'Components.NotificationDropdown.unlock-success',
                '你已成功解鎖「{{title}}」',
                { title: parsedContent.unlockStory.title }
              )}
            </p>,
          ...
          return CommonContainer(
            ...
            <p className="body-3 text-primary-600">
              {t(
                'Components.NotificationDropdown.received-airdrop',
        ...
        if ('title' in parsedContent) {
          return CommonContainer(
            ...
            renderNotifierText(
              ...
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.picked-your-collection',
                  '精選了你的集錦「{{title}}」',
                  { title: parsedContent.title }
                )}
              </span>,
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.all-picked-your-collection',
                  '都精選了你的集錦「{{title}}」',
                  { title: parsedContent.title }
                )}
              </span>
        ...
        if ('title' in parsedContent) {
          return CommonContainer(
            ...
            renderNotifierText(
              ...
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.commented-on-collection',
                  '在你的集錦「{{title}}」下留言',
                  { title: parsedContent.title }
                )}
              </span>,
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.all-commented-on-collection',
                  '都在你的集錦「{{title}}」下留言',
                  { title: parsedContent.title }
                )}
              </span>
        ...
        if ('title' in parsedContent) {
          return CommonContainer(
            ...
            renderNotifierText(
              ...
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.also-commented-on',
                  '也在「{{title}}」下留言',
                  { title: parsedContent.title }
                )}
              </span>,
        ...
        if ('content' in parsedContent) {
          return CommonContainer(
            ...
            renderNotifierText(
              ...
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.liked-your-comment',
                  '喜歡你的留言「{{title}}」',
                  { title: parsedContent.content }
                )}
              </span>,
              <span className="text-primary-600">
                {t(
                  'Components.NotificationDropdown.all-liked-your-comment',
                  '都喜歡你的留言「{{title}}」',
                  { title: parsedContent.content }
                )}
              </span>
  ...
  const getAnnouncementTitle = (type: string | undefined | null) => {
    switch (type) {
      case 'features':
        return t('Components.NotificationDropdown.new-feature', '新功能上線')
      case 'new_media':
        return t('Components.NotificationDropdown.new-publisher', '新媒體上架')
      case 'maintain':
        return t(
          'Components.NotificationDropdown.system-maintenance',
          '系統維修公告'
        )
      default:
        return ''
      ...
      <div className="flex h-dvh flex-col overflow-y-scroll sm:h-auto sm:max-h-[calc(95vh-60px)]">
        <div className="flex h-15 flex-row items-center border-b-[0.5px] p-2 sm:hidden">
          ...
          <h2 className="list-title mx-auto text-primary-800">
            {t('Components.NotificationDropdown.notification', '通知')}
          </h2>
          <button
        ...
        {notification?.current.length ? (
          <>
            <div className="h-15 px-5 pb-3 pt-4">
              <h4 className="list-title">
                {t(
                  'Components.NotificationDropdown.new-notification',
                  '新通知'
                )}
              </h4>
            </div>
          ...
          <>
            ...
            <p className="body-3 px-5 pb-5 text-primary-500">
              {t(
                'Components.NotificationDropdown.no-new-notification',
                '目前沒有新通知...'
              )}
            </p>
          </>
        ...
        {notification?.prev.length ? (
          <>
            <div className="h-15 px-5 pb-3 pt-4">
              <h4 className="list-title">
                {t(
                  'Components.NotificationDropdown.previous-notification',
                  '之前的通知'
                )}
              </h4>
            </div>
