        ...
        <div className="flex flex-row gap-3 overflow-x-scroll">
          {followSuggestions.map((publisher) => {
            ...
            const publisherStatus = isHidden
              ? t('publisherSuggestion.statusHidden', '您不會再收到此推薦')
              : createdAt > Date.now() - 30 * DAY
              ? t('publisherSuggestion.statusNew', '新加入')
              : t(
                  'publisherSuggestion.statusFollowers',
                  '{{followerCount}}人追蹤',
                  { followerCount }
                )
